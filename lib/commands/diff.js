  ...
  try {
    ...
    return {
      ...
      summary: result.isDifferent ? 
        `✗ 다름 (${Differ.formatFileSize(localSize)} vs ${Differ.formatFileSize(remoteSize)})` : 
        '✓ 동일'
    };
  } catch (error) {
    return {
...
async function safeCompareByHash(ssh, localFile, remoteFile) {
  try {
    ...
    return {
      success: true,
      data: result,
      summary: result.isDifferent ? '✗ 다름' : '✓ 동일'
    };
  } catch (error) {
    return {
      success: false,
...
async function safeCompareByContent(ssh, localFile, remoteFile) {
  try {
    ...
    return {
      success: true,
      data: result,
      summary: result.isDifferent ? `✗ 다름 (${result.diff.filter(d => d.added || d.removed).length}줄 차이)` : '✓ 동일'
    };
  } catch (error) {
    return {
      success: false,
...
function displayComparisonResults(results, localPath, remoteFile, localStat, remoteStat) {
  // 요약 섹션
  console.log(chalk.blue('━━━━━━━━━━━━━━━━ 파일 비교 요약 ━━━━━━━━━━━━━━━'));
  console.log(chalk.gray('로컬:'), chalk.white(`${localPath} (${Differ.formatFileSize(localStat.size)}, ${new Date(localStat.mtime * 1000).toLocaleString('ko-KR')})`));
  console.log(chalk.gray('원격:'), chalk.white(`${remoteFile} (${Differ.formatFileSize(remoteStat.size)}, ${new Date(remoteStat.mtime * 1000).toLocaleString('ko-KR')})`));
  console.log('');

  console.log(chalk.gray('크기:  '), results.size.success ? chalk.green(results.size.summary) : chalk.yellow(results.size.summary));
  console.log(chalk.gray('시간:  '), results.date.success ? chalk.green(results.date.summary) : chalk.yellow(results.date.summary));
  console.log(chalk.gray('해시:  '), results.hash.success ? chalk.green(results.hash.summary) : chalk.yellow(results.hash.summary));
  console.log(chalk.gray('내용:  '), results.content.success ? chalk.green(results.content.summary) : chalk.yellow(results.content.summary));

  // 에러가 있는 경우 상세 에러 표시
  const hasErrors = Object.values(results).some(r => !r.success);
  if (hasErrors) {
    console.log('');
    console.log(chalk.yellow('경고: 일부 비교에 실패했습니다.'));
  ...
  if (results.content.success && results.content.data.isDifferent) {
    console.log('');
    console.log(chalk.blue('━━━━━━━━━━━━━━━━ 상세 내용 비교 ━━━━━━━━━━━━━━━'));
  ...
  if (!hasAnySuccess) {
    console.log(chalk.red('결론: 모든 비교가 실패했습니다.'));
    return;
  }
  ...
  if (contentSame && hashSame && sizeSame && !hasTimeDifference) {
    console.log(chalk.green('결론: 파일이 동일합니다.'));
  } else if (contentSame && hashSame && sizeSame && hasTimeDifference) {
    console.log(chalk.yellow('결론: 내용은 동일하지만 수정 시간이 다릅니다.'));
  } else {
    console.log(chalk.yellow('결론: 파일이 다릅니다. push를 권장합니다.'));
  }
  console.log('');
}
