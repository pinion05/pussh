...
const { MAX_FILE_SIZE_FOR_COMPARISON } = require('../utils/constants');
...
async function performComprehensiveComparison(ssh, localPath, remoteFile, localStat, remoteStat, spinner) {
  ...
  // 4. 내용 비교 (가장 느림, 마지막에) - with file size check
  spinner.text = 'Step 4/4: Comparing file contents...';
  results.content = await safeCompareByContent(ssh, localPath, remoteFile, remoteStat.size);
...
function safeCompareByDate(localMtime, remoteMtime) {
  try {
    ...
    return {
      ...
      data: {
        ...result,
        localDate: new Date(localMtime),
        remoteDate: new Date(remoteMtime)
      },
      summary
    };
  } catch (error) {
 ...
 * 안전한 내용 비교 (예외 처리 포함)
 */
async function safeCompareByContent(ssh, localFile, remoteFile, remoteSize) {
  let tempFile = null;

  try {
    // Check file size limit
    if (remoteSize > MAX_FILE_SIZE_FOR_COMPARISON) {
      return {
        success: false,
        error: `File too large for content comparison (${Formatter.formatFileSize(remoteSize)} > ${Formatter.formatFileSize(MAX_FILE_SIZE_FOR_COMPARISON)})`,
        summary: `⚠ Skipped (file too large: ${Formatter.formatFileSize(remoteSize)})`
      };
    }
    ...
    tempFile = Security.createSecureTempFile();
...
function displayComparisonResults(results, localPath, remoteFile, localStat, remoteStat) {
  ...
  console.log(chalk.gray('Local:'), chalk.white(`${localPath} (${Differ.formatFileSize(localStat.size)}, ${Formatter.formatDate(localStat.mtime)})`));
  console.log(chalk.gray('Remote:'), chalk.white(`${remoteFile} (${Differ.formatFileSize(remoteStat.size)}, ${Formatter.formatDate(remoteStat.mtime)})`));
  console.log('');
